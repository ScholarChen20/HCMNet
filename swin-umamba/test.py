...
def Mamba_main():
    ...
    #pred生成路径
    file_dir = os.path.join(mask_pred, config['model'] + '_pred_BUS2_' + str(current_date.strftime("%Y-%m-%d")))
...
def deep_main():
    ...
    # 文件路径
    file_dir = os.path.join(mask_pred, config['model']+'_pred_BUS1_'+str(current_date.strftime("%Y-%m-%d")))
    os.makedirs(file_dir, exist_ok=True)
    file_path = file_dir + "/Metric.xlsx"
