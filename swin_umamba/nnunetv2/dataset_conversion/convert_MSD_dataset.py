...
def convert_msd_dataset(source_folder: str, overwrite_target_id: Optional[int] = None,
    ...
    assert isdir(labelsTs), f"labelsTs subfolder missing in source folder"
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as p:
        ...
        source_images = [i for i in subfiles(imagesTr, suffix='.png', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
        # source_images = [join(imagesTr, i) for i in source_images]

        for s in source_images:
            shutil.copy(join(imagesTr,s),join(target_imagesTr,s))
        # results.append(
        #     p.starmap_async(
        ...
        source_images = [i for i in subfiles(imagesTs, suffix='.png', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
        # source_images = [join(imagesTs, i) for i in source_images]

        for s in source_images:
            shutil.copy(join(imagesTs,s),join(target_imagesTs,s))
        # results.append(
        #     p.starmap_async(
        ...
        # copy segmentations
        source_images = [i for i in subfiles(labelsTr, suffix='.png', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
        for s in source_images:
            shutil.copy(join(labelsTr, s), join(target_labelsTr, s))

        source_images = [i for i in subfiles(labelsTs, suffix='.png', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
        for s in source_images:
    ...
    dataset_json["channel_names"] = dataset_json["channel_names"]
...
def entry_point():
    ...
    parser.add_argument('-i', type=str, required=True,
                        help='Downloaded and extracted MSD dataset folder. CANNOT be nnUNetv1 dataset! Example: '
                             '/home/cwq/MedicalDP/SwinUmamba/data/nnUNet_raw/Data705_Thyroid')
...
if __name__ == '__main__':
    convert_msd_dataset('/home/cwq/MedicalDP/SwinUmamba/data/nnUNet_raw/Data705_Thyroid', overwrite_target_id=705)
